...
def main(config=None):
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=config.data.train.batch_size, shuffle=True,
                                  num_workers=config.data.num_workers, pin_memory=True)
    test_dataloader = DataLoader(test_dataset, batch_size=config.data.test.batch_size, shuffle=False,
                                 num_workers=config.data.num_workers, pin_memory=True)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % config.training.checkpointing_steps == 0:
                    ...
                    save_checkpoint(accelerator, model, global_step, logging_dir,
